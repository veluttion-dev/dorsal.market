...
Esta spec debe vivir sobre `feat/foundation`, porque el sistema de feedback es transversal y no pertenece al slice de usuarios, dorsales ni transacciones.

La implementacion debe hacerse en una rama nueva:

```text
feat/feedback
```

`feat/feedback` debe crearse desde `feat/foundation` y volver a integrarse contra `feat/foundation`. No debe implementarse dentro de `feat/usuarios`.
