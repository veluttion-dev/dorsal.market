...
- `apps/web/lib/auth.ts`: necesario para el mock server-side de Credentials mientras Identity backend no exista en local.
...
- `apps/web/features/transactions/components/checkout-form.client.tsx`: bloquear checkout si el perfil no tiene `runner_data_complete`.
